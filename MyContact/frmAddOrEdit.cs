    ...
    public partial class frmAddOrEdit : Form
    {
        ...
        private void frmAddOrEdit_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                txtMobail.Text = dt.Rows[0][3].ToString();
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            if (ValidateInputs())
            {
                ...
                if (contactId == 0)
                {
                    isSuccess = repository.Insert(txtName.Text, txtFamily.Text, txtMobail.Text, txtEmail.Text, (int)txtAge.Value, txtAddress.Text);
                }
                else
                {
                    isSuccess = repository.Update(contactId, txtName.Text, txtFamily.Text, txtMobail.Text, txtEmail.Text, (int)txtAge.Value, txtAddress.Text);
                }

                if (isSuccess == true)
                {
                    MessageBox.Show("عملیات با موفقیت انجام شدهوراا..", "موفقیت", MessageBoxButtons.OK, MessageBoxIcon.Information);
